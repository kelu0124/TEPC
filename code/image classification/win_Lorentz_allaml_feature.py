...
def dery(x, y, z, n, h, delta, gamma, beta, rk, couple, Laplacian):
    ...
    for i in range(n):
        k1x = h * (delta * (y[i] - x[i]))
        k1y = h * (gamma * x[i] - y[i] - x[i] * z[i] + calculate_couple(i, x, Laplacian, couple) + rk * calculate_cc1(i, x))
        k1z = h * (x[i] * y[i] - beta * z[i])

        k2x = h * (delta * (y[i] + 0.5 * k1y - x[i] + 0.5 * k1x))
        k2y = h * (gamma * (x[i] + 0.5 * k1x) - (y[i] + 0.5 * k1y) - (x[i] + 0.5 * k1x) * (z[i] + 0.5 * k1z) + calculate_couple(i, x, Laplacian, couple) + rk * calculate_cc1(i, x + 0.5 * k1x))
        k2z = h * ((x[i] + 0.5 * k1x) * (y[i] + 0.5 * k1y) - beta * (z[i] + 0.5 * k1z))

        k3x = h * (delta * (y[i] + 0.5 * k2y - x[i] + 0.5 * k2x))
        k3y = h * (gamma * (x[i] + 0.5 * k2x) - (y[i] + 0.5 * k2y) - (x[i] + 0.5 * k2x) * (z[i] + 0.5 * k2z) + calculate_couple(i, x, Laplacian, couple) + rk * calculate_cc1(i, x + 0.5 * k2x))
        k3z = h * ((x[i] + 0.5 * k2x) * (y[i] + 0.5 * k2y) - beta * (z[i] + 0.5 * k2z))

        k4x = h * (delta * (y[i] + k3y - x[i] + k3x))
        k4y = h * (gamma * (x[i] + k3x) - (y[i] + k3y) - (x[i] + k3x) * (z[i] + k3z) + calculate_couple(i, x,Laplacian, couple) + rk * calculate_cc1(i, x + k3x))
...
def main(couple,cutoff):
    ...
    #np.save(r'D:\python\result\allaml\feature\Lorentz/%s_4RK_feature_cutoff_%.3f_couple_%.3f.npy' % (dataset,cutoff,couple), feature)
    np.save(r'/public/home/chenlong666/desktop/my_desk1/coil_20/feature/Lorentz/%s_4RK_feature_cutoff_%.3f_couple_%.3f.npy' % (dataset, cutoff, couple), feature)
    # 打印feature数组的维数
...
if __name__ == '__main__':
    start = process_time()
    dataset = 'allaml'
    #file = np.load(r'D:\python\result\allaml\origin_data/allaml_reduced.npy')
    ...
    eta = 3
    kappa = 1
    #cutoffs = [0.184, 0.208, 0.232, 0.256, 0.28]
    #couples = np.arange(11, 31, 1)
    #cutoffs = float(sys.argv[1])
    couples = np.array([float(sys.argv[1])])
    print("couple: ",couples)
    cutoffs = [float(sys.argv[2])]
