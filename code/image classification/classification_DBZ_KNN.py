...
def expand_pro_matrix(y_pred_pro,y_true,y_pred):
    ...
    pro_exist = {_: False for _ in range(int(max(y_true)) + 1)}
    for sort in y_pred:
        pro_exist[sort] = True
    for sample in range(len(y_pred_pro_new)):
        exist_sort = 0
        for sort in range(int(max(y_true)) + 1):
...
def compute5foldClassification(X_ccp, y_true, dataset, couple,max_state = 10):
    ...
    pro = np.zeros((len(y_true), int(max(y_true)) + 1))
    ...
    for state in range(max_state):
        ...
        y_pred_list_pro = np.zeros((len(y_true),int(max(y_true)) + 1))
    ...
    print(f'couple:{couple},accuracy: {accuracy}', flush=True)
...
def main():
    couples = np.arange(1,13,1)
    for couple in couples:
        dataset = 'allaml'

        # 特征矩阵
        path_feature = r'/public/home/chenlong666/desktop/my_desk1/coil_20/feature/Lorentz/%s_feature_combined_couple_%.3f.npy'%(dataset,couple)
        ...
        print(f'couple:{couple},Balanced Accuracy: {balanced_acc}', flush=True)
