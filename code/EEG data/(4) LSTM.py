...
data_to_use = all_data_standardized  # 你可以尝试改为 all_data_normalized
...
# RNN 模型构建函数（使用堆叠 LSTM 或 GRU，增加更多的层和神经元）
def create_model():
    model = Sequential()

    # 使用堆叠 LSTM 代替 SimpleRNN，增加模型复杂度
    model.add(LSTM(128, activation='tanh', input_shape=(300, 3), return_sequences=True))  # 堆叠的 LSTM
    model.add(Dropout(0.2))  # Dropout防止过拟合
    model.add(LSTM(256, activation='tanh', return_sequences=False))  # 第二层 LSTM
    model.add(Dropout(0.2))  # Dropout防止过拟合

    # 添加一个全连接层
    model.add(Dense(256, activation='relu'))  # 添加隐藏层，增强模型能力
    model.add(Dropout(0.2))  # 增加Dropout避免过拟合

    # 输出层
    model.add(Dense(3, activation='softmax'))  # 三分类，使用 softmax 激活函数
...
for train_index, test_index in kf.split(data_to_use):
    ...
    # 训练模型，增加训练轮次，避免过拟合
    model.fit(X_train, y_train, epochs=50, batch_size=32, verbose=0, validation_data=(X_test, y_test), callbacks=[early_stop, lr_schedule])
